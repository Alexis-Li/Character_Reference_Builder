...
export async function pollGenerateTask(
  ...
  while (true) {
    ...
    if (Date.now() - startTime > MAX_POLL_TIME) {
      return { success: false, execution: "submitted", querySupport: "supported", upstreamRequestId: taskId, statusUnknown: true, error: `${modelName}: status is still unknown after 10 minutes` };
    }
    ...
    try {
      ...
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        return { success: false, execution: "submitted", querySupport: "supported", upstreamRequestId: taskId, statusUnknown: true, error: `${modelName}: status lookup failed after ${MAX_CONSECUTIVE_ERRORS} consecutive network errors` };
      }
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 || response.status === 429 || response.status === 408) {
        ...
        if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
          ...
          return { success: false, execution: "submitted", querySupport: "supported", upstreamRequestId: taskId, statusUnknown: true, error: `${modelName}: status lookup failed - ${errorText || `HTTP ${response.status}`}` };
        }
        interval = Math.min(interval + INTERVAL_STEP, MAX_INTERVAL);
        continue;
      }
      ...
      return { success: false, execution: "submitted", querySupport: "supported", upstreamRequestId: taskId, error: errorMessage };
    }
    ...
    try {
      ...
      consecutiveErrors++;
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        return { success: false, execution: "submitted", querySupport: "supported", upstreamRequestId: taskId, statusUnknown: true, error: `${modelName}: status lookup returned an invalid response` };
      }
      continue;
    }
