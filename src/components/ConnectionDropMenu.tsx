...
import { isMinimumLoopNodeType } from "@/config/minimumLoop";
...
export const ALL_NODE_OPTIONS: MenuOption[] = (() => {
  ...
  return all
    .filter((option) => isMinimumLoopNodeType(option.type))
...
export function ConnectionDropMenu({
  ...
  const getOptions = useCallback((): MenuOption[] => {
    if (!handleType || !["image", "text"].includes(handleType)) return [];
    ...
    const withSaved = (options: MenuOption[]): MenuOption[] => [
      ...options.filter((option) => option.isAction || isMinimumLoopNodeType(option.type)),
      ...saved,
    ];
