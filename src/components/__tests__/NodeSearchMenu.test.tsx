...
describe("NodeSearchMenu", () => {
  ...
  it("filters the list by the search query", () => {
    ...
    expect(labels.every((l) => /generate image/i.test(l))).toBe(true);
    expect(screen.getByText("Generate Image")).toBeInTheDocument();
  });
  ...
  describe("saved Comfy nodes", () => {
    ...
    it("lists them after the built-ins, under their own heading", () => {
      ...
      expect(buttons).toHaveLength(ALL_NODE_OPTIONS.length);
  ...
  it("selects the highlighted option on Enter", () => {
    ...
    fireEvent.change(input, { target: { value: "generate" } }); // narrows to one result
