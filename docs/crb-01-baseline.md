...
产品只覆盖图像参考的输入、理解、生成、编辑、比较与输出。音频和视频永久不属于产品能力；底座中的相关节点、执行器、API、依赖和测试因上游快照复现与分步清理仍可能存在，但它们不是待启用功能，也不得重新暴露为 UI、快捷键、工作流或 Provider 入口。

LLM 与 ComfyUI 不属于这批裁剪项：LLM 服务于视觉理解、推理和业务任务，ComfyUI 作为可选执行后端按实际闭环价值继续评估。社区与多 Provider 能力同样按产品需求独立取舍。首个导航只暴露原画输入、标注、Prompt、图像生成、图像比较、候选画廊和输出；外部 3D、社区模板和全量模型浏览不进入首批导航。
