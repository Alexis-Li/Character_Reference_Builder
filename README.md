...
操作与验证证据见 [CRB-05 验证记录](docs/validation/part-reference-issue-6.md)。真实素材和模型效果仍待用户验收。

## 当前验证状态

固定 Node Banana 基线的干净安装、生产构建、123 个测试文件共 2598 项测试和 Windows 本机启停已经通过。导入前的需求探针仍记录 P04–P07 四项业务缺口；真实模型质量、费用和完整生产闭环尚未通过。详见验证报告和 CRB-01 基线说明。
